...
describe('studentEngagementReceived adapter — fromDbWire', () => {
...
describe('studentEngagementReceived adapter — toDbWire', () => {
  it('returns a StudentEngagementReceivedDbWire instance', () => {
    const result = toDbWire({ id: eventId, journeyId });
    expect(result).toBeInstanceOf(StudentEngagementReceivedDbWire);
  });
