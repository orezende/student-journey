...
export const analysisFinished = asyncFn(Event, async (event) => {
  ...
  if (!previous) throw new NotFoundError('AnalysisFinished not found for eventId: ' + event.eventId);
  ...
  await journeyDb.updateStep(buildJourneyStepUpdate({ id: previous.journeyId, currentStep: 'CURRICULUM_GENERATED' }));
