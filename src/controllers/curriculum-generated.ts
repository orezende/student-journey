...
export const curriculumGenerated = asyncFn(Event, async (event) => {
  ...
  if (!previous) throw new NotFoundError('CurriculumGenerated not found for eventId: ' + event.eventId);
  ...
  await journeyDb.updateStep(buildJourneyStepUpdate({ id: previous.journeyId, currentStep: 'CONTENT_DISPATCHED' }));
