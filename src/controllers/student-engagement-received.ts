...
export const studentEngagementReceived = asyncFn(Event, async (event) => {
  ...
  if (!previous) throw new NotFoundError('StudentEngagementReceived not found for eventId: ' + event.eventId);
  ...
  await journeyDb.updateStep(buildJourneyStepUpdate({ id: previous.journeyId, currentStep: 'PROGRESS_MILESTONE_REACHED' }));
