...
export const progressMilestoneReached = asyncFn(Event, async (event) => {
  ...
  if (!previous) throw new NotFoundError('ProgressMilestoneReached not found for eventId: ' + event.eventId);

  const existing = await journeyCompletedDb.findById(event.eventId);
  if (existing) return;

  await journeyCompletedDb.insert(buildEventRecord(event));
  await journeyDb.updateStep(buildJourneyStepUpdate({ id: previous.journeyId, currentStep: 'JOURNEY_COMPLETED' }));
  await journeyDb.updateStatus(buildJourneyStatusUpdate({ id: previous.journeyId, status: 'completed' }));
});
